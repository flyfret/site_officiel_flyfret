    ...
    public function return(Request $request)
    {
        ...
        if ($paiement) {
            return redirect()->route('payment.success')->with([
                'message' => 'Votre paiement a été validé et l\'expédition enregistrée avec succès !',
                'transaction_id' => $transaction_id
            ]);
        }
        ...
        if ($expeditionData) {
            try {
                ...
                // Même en cas d'erreur, on redirige vers le succès car le paiement a été effectué
                return redirect()->route('payment.success')->with([
                    'message' => 'Votre paiement a été validé ! L\'expédition sera enregistrée sous peu.',
                    'transaction_id' => $transaction_id
                ]);
            }
        }

        // Si les données ne sont pas trouvées, on affiche quand même le succès
        return redirect()->route('payment.success')->with([
